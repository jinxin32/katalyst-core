...
func matchTimeSlot(slot v1alpha1.IndicatorTimeTargetSlot) (float64, bool) {
	...
	if err != nil {
		general.Warningf("failed to parse start time %v in time slot: %v", slot.TimeRange[0], err)
		return 0, false
	}
	end, err := time.ParseInLocation(layout, string(slot.TimeRange[1]), loc)
	if err != nil {
		general.Warningf("failed to parse end time %v in time slot: %v", slot.TimeRange[1], err)
		return 0, false
	}
